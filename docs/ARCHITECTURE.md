...
DPAPI는 storage의 SecretStore 인터페이스로 격리한다. 서버·CLI 런타임은 비동기 PowerShell 헬퍼를 숨겨 실행하고 10초 timeout·1 MiB 출력 상한을 적용한다. 비밀값은 명령 인수가 아닌 stdin으로 전달한다. `secrets.json` 쓰기는 프로세스 간 잠금 파일과 임시 파일 교체로 직렬화한다. 현재 계정 전용 파일 ACL을 적용한다. 키 조회 API는 마스킹 정보만 반환한다. DPAPI 실패 시 평문 저장으로 대체하지 않는다. 기존 동기 구현은 시험 호환용으로 남겨 둔다.
...
파일 도구는 realpath 및 각 경로 요소를 확인한다. scoped 정책은 작업 폴더 기준 상대 경로와 그 범위 내부 절대 경로를 같은 후보로 정규화하고, 허용 작업 폴더 밖·다른 드라이브·UNC 경로를 거부한다. 실제 root는 MCP·모델 설명에 주입하지 않고 도구 결과 경로는 작업 폴더 기준 상대 표기를 사용한다. 모든 정책에서 심볼릭 링크/junction을 거부한다. full 정책은 단일 root를 두지 않고 모든 파일 경로와 명령 cwd에 절대 경로를 요구하며 현재 OS 사용자 권한으로만 접근한다. 새 파일만 해시 없이 생성할 수 있다. 기존 파일의 쓰기·텍스트 교체는 직전 읽기에서 얻은 기대 해시를 필수로 받고, 누락이나 불일치를 변경 전에 거부한다. 쓰기 시 부모 경로를 다시 확인한다. 이 방어는 악의적인 동시 파일시스템 교체까지 격리하는 OS sandbox가 아니다.

scoped 작업은 같거나 포함 관계인 workspace끼리 직렬화한다. 포함 판정은 `path.relative` 결과가 정확히 `..`이거나 `..` 뒤 경로 구분자로 시작할 때만 상위 이동으로 보며, `..cache` 같은 합법적 하위 폴더 이름도 직렬화한다. full 작업은 모든 scoped/full 파일 작업과 전역 직렬화해 서로 다른 드라이브·UNC·기준 폴더 표현으로 동일 파일의 잠금을 우회하지 못하게 한다.

명령 도구는 기본 꺼짐이다. 모델 도구 정의에는 허용된 commandId enum과 표시 label만 제공하고 executable 경로는 노출하지 않으며, 허용 목록이 비어 있으면 `run_command` 자체를 제공하지 않는다. 사용자가 활성화한 에이전트에서 허용한 실행 파일 절대 경로와 인자 배열을 `shell:false`로 실행한다. 부모 `process.env`는 상속하지 않는다. Windows는 `SystemRoot`, `WINDIR`, `ComSpec`, `PATHEXT`, `PATH`, `TEMP`, `TMP`, POSIX는 `PATH`, locale(`LANG`, `LC_*`)과 `TMPDIR` 중 부모에 실제 존재하는 값만 새 환경 객체로 전달한다. `HOME`, 사용자 프로필, API key, token, `NODE_OPTIONS`와 기타 서비스 변수는 전달하지 않는다. 사용자 지정 환경값은 v1 설정 계약에 포함하지 않는다. Windows 프로세스 트리 종료 헬퍼에도 같은 최소 환경을 사용한다.

Windows 명령 취소는 OS 프로세스 목록의 PID·부모 PID·생성 시각으로 트리를 식별하고 원 명령 PID의 생성 시각을 재확인한 뒤 `taskkill /t /f`를 시도한다. 종료 후 모든 저장된 조상 기준으로 새 후손도 확인하고 실패 기록에 보존한다. 현재 재사용된 PID의 생성 시각 이후에 태어난 프로세스는 이전 조상의 후손으로 편입하지 않는다. 조회·종료 확인 실패 시 작업 폴더 차단을 큐에 즉시 적용하고 `settings.unsafeCommandBlocks`에 기록한다. 시작 시 이 기록을 다시 읽어 적용한다. 관리 API는 새로 발견한 후손을 영속 저장하지만 빈 스냅샷도 전체 종료 증명이 아니므로 차단을 해제하지 않는다. 사용자의 독립 OS 확인을 요구하는 수동 해제만 허용한다. 원자적 Windows Job Object 추적은 사용하지 않으므로 관측 사이의 미관측 조상 및 PID 검사와 종료 사이의 짧은 경합은 별도 한계다.
...
비밀값 변경은 데이터 폴더의 `secrets.lock`을 단일 작성자 잠금으로 사용하고, 비밀 파일의 읽기·변경·원자적 교체 전체를 잠금 안에서 수행한다. 생성 중 빈/부분 잠금이나 종료된 PID의 잠금은 자동 회수하지 않는다. 소유권을 확실히 구분하지 못하는 파일을 조회 후 삭제하면 다른 작성자의 새 잠금까지 제거할 수 있기 때문이다. 획득 대기는 최대 10초이며 이후 `SECRET_STORE_LOCKED`로 실패한다. 작성자가 비정상 종료돼 잠금이 남으면 서비스와 같은 데이터 폴더를 사용하는 CLI를 모두 중지하고 프로세스 종료를 확인한 다음 `secrets.lock`을 보관 위치로 옮겨 복구한다. `secrets.json`은 이동·삭제하지 않는다. 서비스를 실행한 상태에서 잠금을 수동 제거하면 동시 변경을 보호할 수 없다.
...
실행 본문·이벤트 기본 보존 30일, 사용자 조정 1~365일. 만료 terminal 실행은 시작·설정 변경·1시간 주기 작업에서 100건씩 정리하며 배치 사이 이벤트 루프에 양보한다. 진행 중 데이터는 정리하지 않는다. 이벤트에는 단조 증가 seq를 부여하고 SSE는 100건씩 읽는다. DB 외 파일 산출물은 상대 참조만 저장하고 삭제 시 참조를 확인한다.
