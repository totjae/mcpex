...
## 2026-09-25 UX16 실제 서비스 티어 자격·청구 검증

- OpenAI 직접 연결의 `service_tier` 요청/응답 매핑은 공식 문서와 모의 API로 확인했다. 실제 사용자 API 계정, 프로젝트 정책 및 선택 모델에서 `auto/default/flex/priority`의 허용 여부는 검증하지 않았다. 실제 호출 때 거부·응답 실제 티어·요금·지연을 별도로 확인해야 하며 모의 결과로 비용이나 속도 개선을 보장하지 않는다.
- OpenAI 호환 어댑터를 공유하는 다른 공급자 및 Anthropic·Gemini·Vertex·Bedrock의 독립 서비스 티어 계약은 조사·구현하지 않았다. 공식 공급자 문서와 계정별 제약을 확인하기 전까지 전용 선택은 비활성이다. 기존 고급 `extraBody`는 호환성을 위해 보존한다.
- 실제 사용자 DB v6→v7 마이그레이션 및 운영 중 재시작 후 화면·MCP 재연결은 수행하지 않았다. 자동 테스트는 새 DB/모의 데이터에서 마이그레이션·요청 계약을 확인했다.
...
| 항목                    | 현재 상태                | 필요한 검증                                                                                                                          |
| ----------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ |
| 파일 도구 코어          | 모의 통합 검증 완료      | scoped 상대·범위 내부 절대 path와 상대 결과 표기 통과; 실제 사용자 workspace, UNC·다중 드라이브와 장시간 RSS는 미검증                |
| junction·경쟁 경로 방어 | 구현·부분 검증           | root·중간 상위 junction과 도구 호출 시 root 재검사 통과; 경로 요소 동시 교체 경쟁, UNC·다중 드라이브 실환경 확인 필요               |
...
| 외부 영향 범위 안내     | 자동 검증 완료           | UI와 적용 버전 MCP 도구 설명에 파일 결과의 모델 제공자 전송, 클라우드 외부 전송 가능성, 명령의 현재 OS 사용자 권한·비-sandbox를 표시 |
